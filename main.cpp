...
int main()
{
    ...
    uint32_t AmsterdamFounded = foundedMap[CityRecord{ "Amsterdam",500000, 2.5, 9.5 }];
    ...
    {
        const CityRecord& cairoData = cityMap["Cairo"];
    }
    ...
    for (auto& kv : cityMap)
    {
        const std::string& name = kv.first; //the name is the key
